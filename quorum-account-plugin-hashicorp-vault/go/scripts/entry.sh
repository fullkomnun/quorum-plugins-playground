...
#    ./vault.sh
    ...
    roleId=$(curl -v -s \
    --header "X-Vault-Token: qwerty" \
    http://vault-server:8200/v1/auth/approle/role/basicrole/role-id | jq -r '.data.role_id')
    export HASHICORP_ROLE_ID=${roleId}

    secretId=$(curl -v -s \
    --header "X-Vault-Token: qwerty" \
    --request POST \
      http://vault-server:8200/v1/auth/approle/role/basicrole/secret-id | jq -r '.data.secret_id')
...
# EOF
    ...
    http://vault-server:8200/v1/auth/approle/login
